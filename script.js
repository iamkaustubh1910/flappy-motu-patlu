...
const patluPopup = document.getElementById('patlu-popup');
...
function init() {
    ...
    motu.style.left = '100px';
    motu.style.top = position + 'px';
...
// Update Motu's position
function updateMotu() {
    velocity += GRAVITY;
    position += velocity;
    motu.style.top = position + 'px';

    // Rotate Motu based on velocity
    const rotation = Math.min(Math.max(velocity * 3, -90), 45);
    motu.style.transform = `rotate(${rotation}deg)`;
...
function checkCollisions() {
    const motuRect = motu.getBoundingClientRect();
    ...
    pipes.forEach(pipe => {
        ...
        if (
            motuRect.right - collisionBuffer > topPipeRect.left &&
            motuRect.left + collisionBuffer < topPipeRect.right &&
            (motuRect.top + collisionBuffer < topPipeRect.bottom ||
             motuRect.bottom - collisionBuffer > bottomPipeRect.top)
        ) {
            gameOver();
        }
    });
...
// Show Patlu popup
function showPatluPopup() {
    patluPopup.style.display = 'block';
    patluPopup.style.left = '150px';
    patluPopup.style.top = '200px';
    setTimeout(() => {
        patluPopup.style.display = 'none';
    }, 1000);
}
...
function restartGame() {
    ...
    motu.style.top = position + 'px';
    motu.style.transform = 'rotate(0deg)';
